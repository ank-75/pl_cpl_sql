...
begin 
  ...
  execute /*##db=db_PG##*/ 'CREATE TABLE if not exists test.n_medic_regs_dub (
    "id"                              BIGINT NOT NULL,
    "code"                            VARCHAR(30) ,
    ...
    "atx_code"                        VARCHAR(7) ,
    "gnvlp"                           NUMERIC(1,0),
 CONSTRAINT N_MEDIC_REG_PK PRIMARY KEY ("id")
)
'; 
 
  execute /*##db=db_PG##*/ 'delete from test.n_medic_regs_dub';

  dbms_output.put_line('patt = '||patt);
  i := 0; 
  for rec in cur_main_ora loop 
      -- Выполним функцию в БД и вернем ее результат 
      f_name := apt.n_get_reg_name/*##db=db_Oracle##*/(rec.id); 
      -- 
      i := i + 1; 
      dbms_output.put_line(i||') Name = '||f_name||', meas_name='||apt.n_get_measure_name/*##db=db_Oracle##*/(rec.measure_id));
      --record_io.print_record(rec);
      insert into /*##db=db_PG##*/ test.n_medic_regs_dub values(rec.*); 
  end loop;
