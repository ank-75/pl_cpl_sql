...
Datum pl_cpl_sql_call_handler(PG_FUNCTION_ARGS) {
    ...
    PG_TRY();
    {
        ...
        if (prorettype == VOIDOID)
        {
            ...
            //ReleaseSysCache(pl_tuple); // Освобождаем кэш
...
Datum pl_cpl_sql_inline_handler(PG_FUNCTION_ARGS) {
    ...
    //---Проницициализируем структуру для возврата значения----
    THandlerResultRec* result_struct = (THandlerResultRec*)palloc(sizeof(THandlerResultRec));

    result_struct->Errors = (char*)palloc(100 * sizeof(char)); // Выделяем память для строки
    result_struct->Messages = (char*)palloc(100 * sizeof(char)); // Выделяем память для строки
